...
export class Terrain<M extends Material> extends BatchedMesh {
  ...
  constructor(material: M, options: TerrainParams) {
    ...
    const maxIndexCount =
      maxInstanceCount * chunkVertexIndex * multiplierLODReservedIndex;
  ...
  public async addChunk(x: number, z: number): Promise<void> {
    ...
    this.setMatrixAt(
      instanceId,
      matrix4.setPosition(x * chunkSize, 0, z * chunkSize)
    );
  ...
  protected generateChunkGeometry(x: number, z: number): PlaneGeometry {
    ...
    for (let o = 0; o < octaves; o++) {
      for (let i = 0; i < vertices.length; i++) {
        vertices[i * 3 + 1] +=
          this.noiseCallback(
            (vertices[i * 3] + x) * frequency,
            (vertices[i * 3 + 2] + z) * frequency
          ) * amplitude;
      }
  ...
  protected async addGeometries(geometry: PlaneGeometry): Promise<number> {
    ...
    const geometryId = this.addGeometry(
      geometry,
      -1,
      (geometry.index?.count ?? 0) * multiplierLODReservedIndex
    );
  ...
  protected async createLODs(
    ...
    for (let i = 0; i < LODCount; i++) {
      geometries.push(
        await simplifyGeometry(geometries[i - 1] ?? geometry, {
          ratio: 0.5,
          lockBorder: true,
        })
      );
  ...
  public async generateTrees(count: number): Promise<Vector3[]> {
    ...
    for (let i = 0; i < count; i++) {
      ...
      for (let o = 0; o < octaves; o++) {
        noiseVal +=
          this.noiseCallback(
            x * chunkSize * frequency,
            z * chunkSize * frequency
          ) * amplitude;
        amplitude *= this.gain;
        frequency *= this.lacunarity;
      }

import { Material, Matrix4, Mesh, PlaneGeometry, Sphere } from "three";
import { computeObjectBoundingSphere } from "../utils/computeObjectBoundingSphere";
import {
  type CreateOctahedralImpostor,
  createOctahedralImpostorMaterial,
} from "./octahedralImpostorMaterial";

export class OctahedralImpostor<M extends Material = Material> extends Mesh<
  PlaneGeometry,
  M
> {
  constructor(materialOrParams: M | CreateOctahedralImpostor<M>) {
    ...
    if (!(materialOrParams as M).isOctahedralImpostorMaterial) {
      ...
      materialOrParams.transform = new Matrix4()
